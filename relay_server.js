...
let chainsplainResponse = '';
function delay(duration) {
  return new Promise((resolve) => {
    setTimeout(resolve, duration); // duration in milliseconds
  });
}
...
const confirmAndSend = async (req) => {
  ...
  delay(1000).then(() => {
    // Code to be executed after the delay of 1 second
    console.log("Delayed 1 second");
  });

  try {
      // This may need to be a JSON object
      const signature = req.body[0];
      const searchQuery = req.body[1];
      
      // const data = await SOLANA_CONNECTION.getTransaction(signature);
      // test and then retry 3 times if the tx is not confirmed
      let retryCount = 0;
      let data = null;
      while (retryCount < 3 && data === null) {
        try {
          data = await SOLANA_CONNECTION.getTransaction(signature);
        } catch (err) {
          console.log(`Error: Transaction not yet Confirmed: ${err}`);
          retryCount++;
          delay(10000).then(() => {
            // Code to be executed after the delay of 1 second
            console.log("Delayed 10 second");
          });
        }
      }
      ...
      if (data === null) {
        ...
        return "API request failed after 3 attempts";
        // Handle the case when the value is unavailable
      } else {
        // Process the data when it is available
        // ...
        ...
        try {
          if (sigCheck && errCheck && lamportsCheck && destinationCheck && timeoutCheck) {
